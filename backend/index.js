...
const celeryTasks = {
  crawlGov: "crawler.tasks.crawl_gov",
  crawlGovDu: "crawler.tasks.crawl_gov_du",
  crawlGovMp: "crawler.tasks.crawl_gov_mp",
};
...
router.post("/crawler/run", async (ctx) => {
  const config = ctx.request.body;

  switch (config.regex) {
    case "dziennikustaw.gov.pl":
      var taskName = celeryTasks.crawlGovDu;
      break;
    case "monitorpolski.gov.pl":
      var taskName = celeryTasks.crawlGovMp;
      break;
    case "*.gov":
      var taskName = celeryTasks.crawlGov;
      break;
    default:
  ...
  if (taskName === undefined) {
    ctx.throw(400, "Invalid website regex");
    return;
  }
  ...
  const result = task.applyAsync(); // pass "[config]" as argument to pass configuration to the task
...
function constructParams(body, documentType) {
  let params = {
    ...
    body: {
      query: {
        bool: {
          must: [{ match: { document_type: documentType } }],
        },
      },
    },
  };
  ...
  if (body.infoDateTo.length > 0 && body.infoDateFrom.length > 0) {
    params.body.query.bool.must.push({
      range: { info_date: { gte: body.infoDateFrom, lte: body.infoDateTo } },
    });
  }
